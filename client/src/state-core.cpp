...
void StateCore::render() {
    ...
    // full-screen window with the scene
    {
        ...
        if (game.size() == 2) {
            drawPlayer(0, ImVec2(0, -1));
            drawPlayer(1, ImVec2(0, 1));
        }

        if (game.size() == 4) {
            drawPlayer(0, ImVec2(0, -1));
            drawPlayer(1, ImVec2(1,0));
            drawPlayer(2, ImVec2(0, 1));
            drawPlayer(3, ImVec2(-1, 0));
        }
...
void StateCore::drawCard(PlayerIndex playerIndex, CardIndex cardIndex, ImVec2 pos, ImVec2 dir) {
    auto& player = game.getPlayer(playerIndex);
    auto& card = game.getPlayer(playerIndex).getHand().getCard(cardIndex);
    ...
    if (card.state() == Card::State::IN_PLAY) {
        dir.x = 0;
        dir.y = 1;
        pos.x = (wSize.x - handSize.x) / 2 + card.getTrickIndex() * (cardSize.x + cardPadding);
    ...
    if (!player.isAI() && ImGui::IsItemHovered() && ImGui::IsMouseJustPressed(0) && card.state() == Card::State::INIT && game.getCurrentPlayer() == playerIndex) {
        if (connectionHandler_ && connectionHandler_->connected() && game.connected())
            connectionHandler_->sendMove(Move(playerIndex, cardIndex, 0, 0));
...
void StateCore::drawHand(PlayerIndex playerIndex, ImVec2 pos, const ImVec2& dir) {
...
void StateCore::drawToepButton(const ImVec2& pos, const ImVec2& dir) {
    ...
    if (ImGui::IsItemClicked()) {
        ...
        if (connectionHandler_ && connectionHandler_->connected() && game.connected())
            connectionHandler_->sendToep();
        game.toep(0);
    }
}

void StateCore::drawConnectButton()
{
    ...
    if (game.connected()) {
        ...
        if (ImGui::Button("Connect to Server")) {
            connectionHandler_ = std::make_unique<ConnectionHandler>(game, gameMutex_);
        }
    }

    if (connectionHandler_ && !connectionHandler_->connected()) {
...
void StateCore::drawPlayer(PlayerIndex playerIndex, const ImVec2& dir) {
    const ImVec2& wSize = rendering.wSize;
    ...
    if (!game.getPlayer(playerIndex).isAI()) {
        ...
        drawCallFoldButton(scorePos, ImVec2(0, 1));
        drawConnectButton();
    }
}

void StateCore::drawCallFoldButton(const ImVec2& pos, const ImVec2& dir) {
    if (game.getState() != Game::State::TOEP || game.getCurrentToeper() != 0) return;
    ...
    if (ImGui::Begin("Decision##Popup", nullptr,
        ...
        if (ImGui::Button("Fold", ImVec2(120, 50))) {
            if (connectionHandler_ && connectionHandler_->connected() && game.connected())
                connectionHandler_->sendFoldCall(false);
            game.playToep(0, false);
        }
        ...
        if (ImGui::Button("Call", ImVec2(120, 50))) {
            if (connectionHandler_ && connectionHandler_->connected() && game.connected())
                connectionHandler_->sendFoldCall(true);
            game.playToep(0, true);
        }
